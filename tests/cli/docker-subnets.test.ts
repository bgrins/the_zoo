...
import { checkDocker, execCommand } from "../../cli/lib/utils/docker";
import { getDockerSubnets } from "../../cli/lib/utils/network-env";

vi.mock("../../cli/lib/utils/docker", () => ({ checkDocker: vi.fn(), execCommand: vi.fn() }));
...
function fakeDocker(listings: string[][]) {
  let calls = 0;
  vi.mocked(execCommand).mockImplementation(async (_command, args) => {
...
describe("getDockerSubnets", () => {
  ...
  it("should find no subnets only when Docker is unavailable", async () => {
    vi.mocked(execCommand).mockRejectedValue(new Error("Cannot connect to the Docker daemon"));
