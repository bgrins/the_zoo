...
```bash
docker compose exec -T postgres pg_dump --no-acl --restrict-key=zoo {flags} -U {service}_user {service}_db > core/postgres/seed/{file}
```

`--restrict-key=zoo` replaces the random key pg_dump writes into each dump. The flags leave out the session, login-token and audit rows that logins create, so repeated captures don't churn (the tables stay, empty):

| `{service}` | `{file}`           | `{flags}`                                                                                    |
| ----------- | ------------------ | -------------------------------------------------------------------------------------------- |
| auth        | `auth.sql`         | `--exclude-table-data='public.hydra_oauth2_*'` (Hydra's login sessions, consents and tokens) |
...
```bash
docker compose exec -T postgres pg_dump --no-acl --restrict-key=zoo -U stalwart_user stalwart_db \
